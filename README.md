...
After importing the data into my excel sheet, i took several steps to calculate various metrics in determining the success of the apps with the data provided. I calculated the percentage of users activation rate by dividing the total installs by user sign ups. The Average and Sum functions was used to calculate engagement metrics and the average time spent on the app per user in the track period using the required columns for both apps. Crash and uninstall rate for each day was calculated and the columns was provided. In addition, i grouped the date column into weekly basis for improved efficiency and quality of data.
